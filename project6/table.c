...
//Runtime: O(n)
//Adds new elements in array and sorts
	assert(data != NULL);
	int pivot;
	pivot = lo;
	int i;
...
//Runtime: O(nlogn)
//Uses recursion to quicksort data
	assert(data != NULL);
	if(lo >= hi){
		return;
	}
	int mid;
	mid  = partition(data, lo, hi, compare);
	quicksort(data, lo, mid-1, compare);
	quicksort(data, mid + 1, hi, compare);
}
